(function () {
  ...
  function getConfig$1() {
    return {
      ...
      proxy: LME_PROXY_URL,
      user: sanitizeConfigString(Lampa.Storage.field("".concat(CONFIG_PREFIX$1, "User"))),
      pass: sanitizeConfigString(Lampa.Storage.field("".concat(CONFIG_PREFIX$1, "Pass")))
    };
  }
  ...
  function loginQbittorrent$1() {
    var config = getConfig$1();
    if (!config.user || !config.pass) {
      return Promise.resolve(false);
    }
    return $.ajax({
      url: "".concat(getApiBase$1(), "/api/v2/auth/login"),
      method: "POST",
      ...
      headers: getHeaders$1("application/x-www-form-urlencoded; charset=UTF-8")
    }).then(function (response) {
      return String(response).replace(/^\s+|\s+$/g, "") === "Ok.";
    });
  }
  ...
  function auth$1(showNotification) {
    ...
    if (!config.url) {
      ...
      return Promise.reject(new Error("qBittorrent URL is not configured"));
    }
    var loginStep = config.user && config.pass ? loginQbittorrent$1() : Promise.resolve(true);
    return loginStep.then(function (loginOk) {
      if (!loginOk) {
      ...
      console.log("TDM", "qBittorrent auth error:", error);
      if (showNotification) {
        Lampa.Bell.push({
          text: Lampa.Lang.translate("AuthDenied")
        });
      }
      throw normalizeError(error, "qBittorrent auth failed");
    });
  }
  function GetData$1() {
  ...
  function checkQbittorrentHost$1(ip, port) {
    var url = "http://".concat(ip, ":").concat(port);
    return new Promise(function (resolve) {
      try {
        $.ajax({
          url: url + "/api/v2/app/version",
          method: "GET",
          timeout: 1500
        }).then(function () {
          resolve(url);
  ...
  function scanInBatches$1(ips, port, batchSize) {
    var found = [];
    var index = 0;
    function runBatch() {
      ...
      return Promise.all(batch.map(function (ip) {
        return checkQbittorrentHost$1(ip, port);
  ...
  function scanSubnets$1(prefixes, port) {
    var ips = [];
    ...
    return scanInBatches$1(ips, port, 40);
  }
  function scanQbittorrentNetwork$1(manifest) {
    var port = 8080;
    var manualSubnet = sanitizeConfigString(Lampa.Storage.field(manifest.component + "qBittorentScanSubnet"));
    var prefixes = manualSubnet ? [manualSubnet.replace(/\.$/, "")] : ["192.168.0", "192.168.1", "192.168.31", "192.168.88", "10.0.0", "10.0.1"];
    Lampa.Bell.push({
      text: "Ищу qBittorrent в сети (" + prefixes.join(", ") + ".x)..."
    });
    scanSubnets$1(prefixes, port).then(function (found) {
      if (!found.length) {
        Lampa.Bell.push({
          text: "Ничего не найдено. Проверь подсеть и CORS-заголовок на сервере."
        });
        return;
      }
      Lampa.Select.show({
        title: "Найденные qBittorrent",
        items: found.map(function (url) {
          return {
            title: url,
            url: url
          };
        ...
        onSelect: function onSelect(item) {
          Lampa.Storage.set(manifest.component + "qBittorentUrl", item.url);
          Lampa.Settings.update();
          Lampa.Bell.push({
            text: "Адрес сохранён: " + item.url
          });
        },
      ...
      console.log("TDM", "qBittorrent scan error:", error);
      Lampa.Bell.push({
        text: "Ошибка поиска в сети"
      });
    });
  }
  var Qbittorent = {
  ...
  function openQbittorrentGuideModal() {
    var enabledController = Lampa.Controller.enabled().name;
    var html = $(
      '<div class="qbt-guide" style="padding: 1.2em 1.5em; max-height: 70vh; overflow-y: auto; line-height: 1.5;">' +
        '<div style="font-size: 1.3em; font-weight: 600; margin-bottom: 0.6em;">Настройка qBittorrent Web UI для Lampa</div>' +
        '<div style="opacity: 0.8; margin-bottom: 1em;">Всё это делается один раз в qBittorrent: Инструменты (шестерёнка) → Настройки → Web UI.</div>' +

        '<div style="font-weight: 600; margin-top: 1em;">1. IP-адрес интерфейса</div>' +
        '<div>Поставь <b>0.0.0.0</b> (или оставь пустым), а не 127.0.0.1 — иначе Web UI не будет виден с других устройств в сети.</div>' +

        '<div style="font-weight: 600; margin-top: 1em;">2. Порт</div>' +
        '<div>Запомни порт, который тут указан (по умолчанию 8080). Именно его нужно вписывать в адрес плагина: <b>http://IP-компьютера:порт</b>.</div>' +

        '<div style="font-weight: 600; margin-top: 1em;">3. Host header validation</div>' +
        '<div><b>Сними галку</b> «Enable Host header validation» (или добавь IP компьютера в список разрешённых) — иначе qBittorrent отказывает в запросах, пришедших не с localhost.</div>' +

        '<div style="font-weight: 600; margin-top: 1em;">4. CORS-заголовки (Add custom HTTP headers)</div>' +
        '<div>Добавь две строки:</div>' +
        '<div style="background: rgba(255,255,255,0.08); border-radius: 0.4em; padding: 0.6em 0.8em; margin: 0.4em 0; font-family: monospace; word-break: break-all;">' +
    ...
    Lampa.Modal.open({
      title: 'Инструкция: qBittorrent + Lampa',
      html: html,
      size: 'medium',
      onBack: function onBack() {
        Lampa.Modal.close();
  ...
  function Main$1(manifest) {
    ...
    Lampa.SettingsApi.addParam({
      ...
      param: {
        type: "button",
        name: manifest.component + "qBittorentGuide"
      },
      field: {
        name: '📖 Инструкция по настройке qBittorrent'
      },
      onRender: function onRender(item) {
    ...
    Lampa.SettingsApi.addParam({
      ...
      onRender: function onRender(item) {
        if (Lampa.Storage.field(manifest.component + 'Select') === "qBittorent") {
          updateSecretLabel(item, 'Password', manifest.component + "qBittorentPass");
          item.show();
        } else item.hide();
      },
      onChange: function onChange() {
        openSecretEditor(manifest.component + "qBittorentPass", 'qBittorrent Password');
      }
    });
    Lampa.SettingsApi.addParam({
      component: manifest.component,
      param: {
        name: manifest.component + "qBittorentScanSubnet",
        type: "input",
      ...
      field: {
        name: 'Подсеть для поиска (напр. 192.168.1)'
      },
      onRender: function onRender(item) {
        if (Lampa.Storage.field(manifest.component + 'Select') === "qBittorent") {
          item.show();
    ...
    Lampa.SettingsApi.addParam({
      ...
      param: {
        type: "button",
        name: manifest.component + "qBittorentScan"
      },
      field: {
        name: 'Найти qBittorrent в сети'
      },
      onRender: function onRender(item) {
        if (Lampa.Storage.field(manifest.component + 'Select') === "qBittorent") {
          item.show();
      ...
      onChange: function onChange() {
        scanQbittorrentNetwork$1(manifest);
      }
    });
    Lampa.SettingsApi.addParam({
      component: manifest.component,
